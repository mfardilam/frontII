...
//Vamos a renderizar ahora

function renderizarAlbumes(listado) {
    const covers = document.querySelector(".covers");
    //Cada que se ejecute la función se limpie
    covers.innerHTML = "";

    listado.forEach(album => {
        //Usamos un ternario para ponerle color verde cuando tiene like en true <3
        covers.innerHTML += `
            <li data-id="${album.id}">
...
function mostrarDatosEnPerfil() {
    ...
    const likes = document.querySelector("#cant-favoritos");
    cantAlbumes.innerHTML = `${albumesFamosos.length} ${albumesFamosos.length > 1 ? "albumes": "album"}`;
    let cantLikes = 0;
    albumesFamosos.forEach( album => {
        album.like ? cantLikes++ : "";
        }
    );
    likes.innerHTML = `${cantLikes} ${cantLikes > 1 ? "favoritos": "favorito"}`;
}
